...
def get_invalid_ids_from_range(all_ids_from_range: list[str]) -> list[int]:
    invalid_ids_from_range: list[int] = []

    for id_str in all_ids_from_range:
        if len(id_str) % 2 != 0:
            continue

        firstpart, secondpart = id_str[:len(id_str)//2], id_str[len(id_str)//2:]

        if firstpart == secondpart:
            invalid_ids_from_range.append(int(id_str))
    return invalid_ids_from_range
